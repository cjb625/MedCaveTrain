shinyServer = function(input,output,session) {
  ...
  getPrediction <- reactive({
    RefreshRate = .1
    invalidateLater(1000*RefreshRate, session)
    print('test')
    ...
    library(party)
      ...
      htmlData = substring(htmlData,0,nchar(htmlData)-1)
 ...
 #     if (tr.pred>0.5) {
  ...
  #     # print(paste('Conditional Interference Tree Prediction:  Temperature Too High!'))
  #     paste("{payload: true}")
  #   } else {
  #     # print(paste('Conditional Interference Tree Prediction:  Temperature Normal.'))
  #     paste("{payload: false}")
  #   }
  #   
  # })
  ...
  output$jsonoutput <- renderText({
    ...
    if (CurrentPrediction>0.5) {
       print(paste('Conditional Interference Tree Prediction:  Temperature Too High!'))
      msgJson <<- list(payload=TRUE) #paste("{payload: true}")
      paste("{payload: true}")
    } else {
       print(paste('Conditional Interference Tree Prediction:  Temperature Normal.'))
      msgJson <<- list(payload=FALSE) #paste("{payload: false}")
      paste("{payload: false}")
    }
    })
